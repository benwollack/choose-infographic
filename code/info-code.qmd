...
ggplot(data = ben_time,
  ...
  ylim(0, 14) +
  # adjust colors of all elements to match the plot's theme (green)
  # remove grid lines
  # adjust background colors
  theme(
    # lightest color: panel elements so it stands out
    panel.background = element_rect(fill = "#aded98"),
    panel.grid = element_line(color = "#aded98"),
    # intermediate color: background of everything else
    plot.background = element_rect(fill = "#6fc253"),
    legend.background = element_rect(fill = "#6fc253"),
    # darkest color: text elements
    axis.title = element_text(color = "#0e2b04"),
    legend.title = element_text(color = "#0e2b04"),
    legend.text = element_text(color = "#0e2b04")) +
  # creating title
  # renaming axes
  ...
  annotate(
    ...
    # y location of text
    y = 11, 
    # change color to match plot's theme
    color = "#0e2b04",
    # actual text
    label = "Hiking Day", 
...
ggplot(
  data = ben_tree,
  aes(
    area = distance_walked_km,
    fill = lunch_location,
    group = lunch_location,
    subgroup = bus_trips
)) +
  geom_treemap()
```
...
treemap(ben_tree,
          ...
          # remove plot title (redundant, will be on infographic)
          title = "",
...
Distance Walked (km) by Lunch Location and Bus Trips
...
ggplot(
  ...
  # adjust background colors and remove panel gridlines
  theme(
    plot.background = element_rect(fill = "#d98b82"),
    panel.background = element_rect(fill = "#e0b2ad"),
    panel.grid = element_line(color = "#e0b2ad"),
    legend.background = element_rect(fill = "#d98b82"),
    axis.title = element_text(color = "#290602"),
    legend.title = element_text(color = "#290602"),
    legend.text = element_text(color = "#290602")) +
...
For visualization 2, there also aren't that many patterns. The main pattern is how often I ate lunch at each location rather than distance walked, and bus trips didn't have any noteworthy patterns either. To highlight lunch location, I attached color to this variable to make it stand out. I also made the borders thick and used a different font that naturally looks bold while pushing other information off to the side by using less striking fonts and borders.
